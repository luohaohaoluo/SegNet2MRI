...
class BraTS2020(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #     for i in range(4):
        #         img = images[i]
        #         img[label == 0] += np.random.normal(0, 0.01, img[label == 0].shape)
        #         img[label != 0] += 2
        #         images[i] = img
        #
        #     images = (images - images.mean()) / images.std()
...
if __name__ == '__main__':
    ...
    size = (240, 240, 155)
    test_set = BraTS2020(data_path, pre=False, file_path="../../dataset/brats2020/train.txt", transform=transforms.Compose([
        # RandomRotFlip(),
        CenterCrop(size),
        ToTensor()
    ]))
    ...
    mask_segmentation = label[:, :, slice_w]
    color_segmentation = np.zeros((size[0], size[1], 3))
    color_segmentation[mask_segmentation == 1] = [23, 102, 17]
    color_segmentation[mask_segmentation == 2] = [23, 102, 17]
    color_segmentation[mask_segmentation == 3] = [23, 102, 17]
    ax6.imshow(color_segmentation.astype('uint8'), cmap="gray")
    ax6.title.set_text('GT WT')
    ax6.axis('off')

    mask_segmentation = label[:, :, slice_w]
    color_segmentation = np.zeros((size[0], size[1], 3))
    color_segmentation[mask_segmentation == 1] = [255, 0, 0]
    color_segmentation[mask_segmentation == 3] = [255, 0, 0]
    ax7.imshow(color_segmentation.astype('uint8'), cmap="gray")
    ax7.title.set_text('GT TC')
    ax7.axis('off')

    mask_segmentation = label[:, :, slice_w]
    color_segmentation = np.zeros((size[0], size[1], 3))
    color_segmentation[mask_segmentation == 3] = [250, 246, 45]
    ax8.imshow(color_segmentation.astype('uint8'), cmap="gray")
    ax8.title.set_text('GT ET')
    ax8.axis('off')
